...
WalkMe.prototype.identify = function (data) {
  if (!window.WalkMeAPI) {
    ...
    window._walkmeInternals.Segment.preLoadedData = data;

    if (data.userId()) {
      window._walkmeInternals.Segment.userId = String(data.userId());
      window._walkmeInternals.Segment.isAnonUser = !1
    } else {
      window._walkmeInternals.Segment.userId = String(data.anonymousId());
      window._walkmeInternals.Segment.isAnonUser = !0
    }
    window._walkmeInternals.Segment.email = data.email();
    window._walkmeInternals.Segment.traits = data.traits();

    return;
  }

  window.WalkMeAPI.Segment && window.WalkMeAPI.Segment.identify.apply(null, arguments);
};
